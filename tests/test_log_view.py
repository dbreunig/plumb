...
def test_verify_evidence_ok_stale_unverifiable(monkeypatch):
    ...
    good = _d("1", "aaa", "claude", session_id="S", source_path="/x.jsonl", turn_range=[1, 2], evidence_digest=digest)
    bad = _d("2", "aaa", "claude", session_id="S", source_path="/x.jsonl", turn_range=[1, 2], evidence_digest="0" * 64)
    gone = _d("3", "aaa", "claude", session_id="S", source_path="/x.jsonl", turn_range=[10, 12], evidence_digest=digest)

    class FakeSource:
        name = "claude"
        def discover(self, repo_root, since): return []
        def parse(self, ref, since):
            assert since is None and ref.path == "/x.jsonl" and ref.session_id == "S"
            return turns
...
def test_verify_evidence_parse_failure_is_unverifiable(monkeypatch):
    d = _d("1", "aaa", "claude", session_id="S", source_path="/missing.jsonl", turn_range=[0, 0], evidence_digest="0" * 64)
    class Boom:
        name = "claude"
...
def test_verify_evidence_matches_hook_digest_for_oversized_turn(monkeypatch):
    ...
    d = _d("1", "aaa", "claude", session_id="S", source_path="/x.jsonl",
